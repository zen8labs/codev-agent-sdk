"""CodeGraph list_callees tool — wraps ``codegraph callees``."""
...
class ListCalleesObservation(Observation):
    """Observation from list_callees operations."""
...
TOOL_DESCRIPTION = """List functions/methods that a symbol calls (``list_callees``).

* Wraps ``codegraph callees`` — direct outgoing calls
* Use when you need **what a function calls** before editing or reviewing it
* For full flows or architecture, prefer ``codegraph_explore``
* Requires a ``.codegraph/`` index in the project
"""


class ListCalleesExecutor(ToolExecutor[ListCalleesAction, ListCalleesObservation]):
    def __init__(self, working_dir: str):
        self.working_dir = Path(working_dir).resolve()

    def __call__(
        self,
        action: ListCalleesAction,
        conversation: LocalConversation | None = None,  # noqa: ARG002
    ) -> ListCalleesObservation:
        symbol = action.symbol.strip()
        if not symbol:
            return ListCalleesObservation.from_text(
        ...
        command = append_path_flag(
            [binary, "callees", symbol, "-l", str(limit)],
            search_path,
            self.working_dir,
        )
        result = run_codegraph_cli(
            ...
            error_prefix="CodeGraph callees",
        )
        return ListCalleesObservation.from_text(
            text=result.text,
            symbol=symbol,
...
class ListCalleesTool(ToolDefinition[ListCalleesAction, ListCalleesObservation]):
    @classmethod
    def create(cls, conv_state: ConversationState) -> Sequence[ListCalleesTool]:
        ...
        return [
            cls(
                ...
                action_type=ListCalleesAction,
                observation_type=ListCalleesObservation,
...
register_tool(ListCalleesTool.name, ListCalleesTool)
